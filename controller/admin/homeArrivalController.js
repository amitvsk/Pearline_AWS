...
import homeArrivalBanner from "../../model/admin/homeArrivalBanner.js";
...
export const uploadBanner = async (req, res) => {
  try {
    ...
    const doc = await homeArrivalBanner.create({ image: imageUrl });
...
export const getBanner = async (req, res) => {
  try {
    const banner = await homeArrivalBanner.findOne().sort({ createdAt: -1 });
...
export const updateBanner = async (req, res) => {
  try {
    const banner = await homeArrivalBanner.findById(req.params.id);
